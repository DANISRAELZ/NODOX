...
from src.nodos_funcionales.stage5a1_candidate_discovery import run_stage5a1_validation


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run NODOX Stage 5A.1 with strain/proteome-scoped high-recall candidate "
            "discovery and strict benchmark matching."
        )
    )
    ...
    parser.add_argument("--benchmark-mode", choices=["blind", "conditional"], default="blind", help="blind never injects missing targets; conditional may resolve exact missing targets.")
    parser.add_argument("--benchmark-candidate", action="append", default=[], help="Exact UniProt accession or gene identifier. Repeat for multiple benchmark targets.")
    ...
    parser.add_argument("--taxon-resolution-mode", default="online_optional", choices=["online_optional", "cache_first", "offline_only", "local", "api_stub", "auto"])
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = run_stage5a1_validation(
