...
| human_homologs | Parcialmente conectada | user_curated, external_real, external_general, external_stub, cache_possible, raw_local, demo | `data_user/human_homologs.csv` si existe | UniProt human lookup con backfill `configurable_stub_human_homologs_v1` | `6_proxy_or_controlled` | Aunque hay consulta real a UniProt, el resultado puede depender de backfill configurable; se degrada conservadoramente. | Si | Si | Si | Si | Si | Si | Alto | Sustituir stub por ortologia/BLAST/DIAMOND reproducible o curacion manual de seguridad frente al hospedero. |
