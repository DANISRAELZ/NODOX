...
DOC_PATH = PROJECT_ROOT / "docs" / "user_curated_interpretation_closure_audit.md"
FORBIDDEN_DEFAULT_ORGANISMS = ["corynebacterium", "pao1", "h37rv"]
...
def test_user_curated_interpretation_closure_audit_document_covers_required_contract() -> None:
    ...
    for organism in FORBIDDEN_DEFAULT_ORGANISMS:
        assert f"{organism} por defecto" not in text
        assert f"valor por defecto {organism}" not in text


def test_user_curated_interpretation_closure_audit_functional_outputs_are_stable(tmp_path) -> None:
    ranking = _ranking()
    ...
    for column in [
        ...
        assert column in json_records[0]
    ...
    for phrase in [
        ...
        "confianza alta no equivale a prioridad terapeutica alta",
        ...
        "plataforma multiorganismo",
    ]:
        assert phrase in combined

    for organism in FORBIDDEN_DEFAULT_ORGANISMS:
        assert organism not in combined
